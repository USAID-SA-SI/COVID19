...
date: "last updated 6/02/2022 NOTE THAT THIS IS STILL IN PROGRESS"
...
```
##Step 4a: Read in datasets using google drive
```{r}
#Google Sheet (must be google sheet, cannot be XLSX file) 
  #6.2 Google sheets saved here: https://drive.google.com/drive/folders/1HBS-1RoYCXLwct2EbuZisuJ1l_zuAMOA

ANOVA<-read_sheet(as_sheets_id('19636dXmIRiPhTY0xC8l7gXxgDaVL9Mu_WwvbdvLar-Q'))
BROADREACH<-read_sheet(as_sheets_id('1vL_qcq1NLWl3rmC9ZdfPQA9bIv-0FPzoFxVDGfXB1m0'))
GUIDEHOUSE<-read_sheet(as_sheets_id('1mJOlA13K8tM1XtrLdQTVLUGGiRKY9xt_VHs_Nz8X05g'))
GETF_NEXTMILE<-read_sheet(as_sheets_id('1WeXpo1z8giLbpOilX1Vh9XS6kZxbLMlrnjgM5fSK7lY'))
EQUIP<-read_sheet(as_sheets_id('1NlrGoYtzn0Qna1DjPUQv67I9orEuIR4QS7DKtn_gN4s')) #phase out after Dec 2021
ADAPT_RTC <-read_sheet(as_sheets_id('1_sI7FBOkm6zTOmbl9X1FPHqBq7phmVss3H0ze_v9e_Q'), sheet = "RTC") %>% 
  dplyr::mutate(Sub = dplyr::recode(Sub, `N/A` = 'Right to Care'))   #so RTC dataset aligns with others
ADAPT_Aurum <-read_sheet(as_sheets_id('1_sI7FBOkm6zTOmbl9X1FPHqBq7phmVss3H0ze_v9e_Q'), sheet = "Aurum")

ADAPT_FPD <-read_sheet(as_sheets_id('1_sI7FBOkm6zTOmbl9X1FPHqBq7phmVss3H0ze_v9e_Q'), sheet="FPD")

ADAPT_Genesis <-read_sheet(as_sheets_id('1_sI7FBOkm6zTOmbl9X1FPHqBq7phmVss3H0ze_v9e_Q'), sheet="Genesis")

ADAPT_HE2RO <-read_sheet(as_sheets_id('1_sI7FBOkm6zTOmbl9X1FPHqBq7phmVss3H0ze_v9e_Q'), sheet="HE2RO")

ADAPT_IHPS <-read_sheet(as_sheets_id('1_sI7FBOkm6zTOmbl9X1FPHqBq7phmVss3H0ze_v9e_Q'), sheet="IHPS")

ADAPT_NICD <-read_sheet(as_sheets_id('1_sI7FBOkm6zTOmbl9X1FPHqBq7phmVss3H0ze_v9e_Q'), sheet="NICD")

ADAPT_Qode <-read_sheet(as_sheets_id('1_sI7FBOkm6zTOmbl9X1FPHqBq7phmVss3H0ze_v9e_Q'), sheet="Qode")

ADAPT_ReAction <-read_sheet(as_sheets_id('1_sI7FBOkm6zTOmbl9X1FPHqBq7phmVss3H0ze_v9e_Q'), sheet="ReAction!")

ADAPT_Stellenbosch <-read_sheet(as_sheets_id('1_sI7FBOkm6zTOmbl9X1FPHqBq7phmVss3H0ze_v9e_Q'), sheet="Stellenbosch")
...
ADAPT <-bind_rows(ADAPT_RTC,ADAPT_Aurum, ADAPT_FPD,ADAPT_Genesis,ADAPT_HE2RO,
  ...
  #mutate(`31-May-22`= as.numeric(`31-May-22`)) 



PARTNER_TOOLS <-bind_rows(ANOVA,BROADREACH,GUIDEHOUSE, GETF_NEXTMILE, EQUIP, ADAPT) 
...
df2<- PARTNER_TOOLS %>%  tidyr::gather(`Reporting Date`,`Value`, `31-Oct-21`:`31-Dec-23`) %>%    dplyr::mutate(`Reporting Date` =as.character(as.Date(`Reporting Date`, "%d-%b-%y"),"%d-%b-%y")) %>% 
...
dplyr::mutate(uniqueid=paste(Partner, OrgUnit, `Dataelement Code`, `disagg category`, `Disaggregate`, `Reporting Date`, sep= "_")) %>% 
  dplyr::filter(`Reporting Date` <= "2022-05-31") #change this to the current reporting month
...
df2<- PARTNER_TOOLS %>%  gather(`Reporting Date`,`Value`, `44500`:`44926`) %>%  
...
check7 <-df2 %>% 
  dplyr::filter(`Dataelement Code`== "VACC_0.3") %>% 
  tidyr::unnest(Value) %>% 
  dplyr::mutate(`Value`= as.numeric(`Value`)) %>%
...
check9a <-df2 %>% 
  dplyr::filter(`Dataelement Code`== "VACC_0.3" & NumDenom != "Numerator") %>% 
...
#Dataset to check if value changed from previous month  or incorrectly a number for VACC 0.8 (text)
check9b <-df2 %>% 
...
# filter where the current and previous values change
  dplyr::mutate(check = identical(value_prev_month,Value)) %>% 
  dplyr::mutate(`check`= as.character(`check`)) %>% 
  mutate(check = recode(check, `TRUE` = "NO ISSUE", `FALSE` ="Change from Previous Month or a number")) %>% 
  dplyr::filter(`check`== "Change from Previous Month or a number") %>% 
...
Level2 <- bind_rows(check3, check4, check5, check6, check7, check8, check9a, check9b,check10) 
...
##Uf needed To output CSV file directly
